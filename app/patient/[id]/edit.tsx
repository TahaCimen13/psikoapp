...
export default function EditPatient() {
  ...
  const { getPatient, updatePatient } = useDatabase();
  ...
  const load = useCallback(async () => {
    ...
    if (p) {
      setName(p.name);
      setBirthDate(p.birth_date ? new Date(p.birth_date + 'T00:00:00') : undefined);
      setGender(p.gender);
      // Kayıt formatı "telefon / e-posta"; tek parçaysa @ içeriğine göre ayır
      ...
      setSessionFee(p.session_fee ? String(p.session_fee) : '');
  ...
  return (
    ...
    <ScrollView style={{ flex: 1 }} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Danışanı Düzenle</Text>
      ...
      <View style={styles.activeRow}>
        <View style={{ flex: 1 }}>
          <Text style={styles.activeLabel}>{isActive ? 'Aktif danışan' : 'Pasif danışan'}</Text>
          <Text style={styles.activeSub}>
            {isActive
              ? 'Terapi süreci devam ediyor.'
              : 'Süreç tamamlandı/ara verildi. Kayıtlar saklanır, listede sona düşer.'}
          </Text>
        </View>
        <Switch value={isActive} onValueChange={setIsActive} trackColor={{ false: colors.cardBorder, true: colors.accent }} thumbColor="#fff" />
      </View>
...
const styles = StyleSheet.create({
  ...
  content: { padding: spacing.md, paddingTop: safeTop + spacing.sm, paddingBottom: 40 },
  header: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: spacing.lg },
  cancel: { color: colors.textSecondary, fontSize: 15 },
  title: { ...typography.h3 },
  saveBtn: { color: colors.accent, fontSize: 15, fontWeight: '700' },
  activeRow: { flexDirection: 'row', alignItems: 'center', backgroundColor: colors.card, borderRadius: radius.md, padding: spacing.md, borderWidth: 1, borderColor: colors.cardBorder, marginBottom: spacing.md },
  activeLabel: { ...typography.body, fontWeight: '600' },
  activeSub: { color: colors.textMuted, fontSize: 12, marginTop: 2, lineHeight: 16 },
